\def\modulename{pcp}
\def\solutions{false}
\RequirePackage[utf8]{course}
\pcp

\usepackage{Cdefs}

\usepackage[utf8]{inputenc}

\usepackage{listings}

%\newenvironment{flist}{\begin{itemize}}{\end{itemize}}
\newcommand{\itemex}[1]{ & \#1 \\ }

\newenvironment{petitexemple}{
  \begin{tabular}{l@{\hspace*{0.1\linewidth}}p{0.8\linewidth}}
    \mbox{\bf Exemple:} 
}{\end{tabular}}

\newenvironment{petitsexemples}{
  \begin{tabular}{l@{\hspace*{0.1\linewidth}}p{0.8\linewidth}}
    \mbox{\bf Exemples:} 
}{\end{tabular}}


\newenvironment{consigne}[1]%
{\begin{center}\begin{minipage}{13cm}%
{\large #1}\par}%
{\end{minipage}\end{center}\relax}
\def\Question#1#2{\question{}\textbf{(#2)} #1}


\def\nicefrac#1#2{\ensuremath{#1/#2}}
\newcommand{\matrice}[3]{\ensuremath{{({#1}_{i,j})}_{\hspace*{-1ex}\begin{array}{c}\scriptscriptstyle 1\le i \le  #2\\[-1ex]\scriptscriptstyle 1 \le j \le #3\end{array}}}}

\setlength{\itemsep}{0pt}


\usepackage{caption}
\usepackage{subcaption}
\usepackage{units}


\usepackage{tikz}
\usetikzlibrary{arrows}

\tikzset{
  treenode/.style = {align=center, inner sep=0pt, text centered,
    font=\sffamily},
  arn_n/.style = {treenode, circle, white, font=\sffamily\bfseries, draw
=black,
    fill=black, text width=1.5em},
  arn_r/.style = {treenode, circle, red, draw=red, 
    text width=1.5em, very thick},
  arn_x/.style = {treenode, rectangle, draw=black,
    minimum width=0.5em, minimum height=0.5em}
}
\begin{document}

\input{base-donnees}

\newpage


\input{bichaines}

\newpage

\input{calcul-majorite}

\newpage

\input{calendrier}

\newpage

\input{calendrier-suite}

\newpage

\input{championnat}

\newpage

\input{correction-prg}

\newpage

\input{elections-us}

\newpage

\input{jeu-de-la-vie}

\newpage

\input{matrice-alt}



\newpage

\input{matrices-base}

\newpage

\input{matrices-projet}

\newpage

\input{permutations}

\newpage

\input{points-dans-le-plan}


\newpage

\input{points-dans-l-espace}

\newpage

\input{racine-carre}

\newpage

\input{tableaux-bichaines}

\newpage

\input{tableaux-dynamiques}

\newpage

\input{tas}

\newpage

\input{emprunts-de-livres}

\newpage

\input{composition-musicale}

\newpage

\end{document}


