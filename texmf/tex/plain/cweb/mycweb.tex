...
\def\makeolproctokctli{%
  ...
  \else\ifx\nxt{\^\i}fTnum\addF{0x}\else\addF^\fi  % 0x77 -> \T{\^77}
...
\outer\def\N#1#2#3.{% beginning of starred section \N{depth} {number}. {title}.
...
\def\MN#1{\par % common code for \M, \N. Argument is the section number.
...
%\ZZ{titre}{depth}{section}{page}{sanitized title}
%\jobname: nom de base du fichier
%\def\headerfile{jobname.h}
%\def\testfile{test\_\jobname.c}
...
\newcount\thechapter
\newcount\thesection
\newcount\thesubsection
\newcount\thesubsubsection
\newcount\thesubsubsubsection
\newcount\thesubsubsubsubsection
\newcount\thesubsubsubsubsubsection
\def\resetsectioncounters{%
\thesubsubsubsubsubsection=0%
\thesubsubsubsubsection=0%
\thesubsubsubsection=0%
\thesubsubsection=0%
\thesubsection=0%
\thesection=0%
\thechapter=0}
\def\initprinttoc{\resetsectioncounters\line{\hfil Section\hbox to3em{\hss Page}}}
\def\incr#1{%
\ifnum#1=6\global\advance\thesubsubsubsubsubsection 1
\else
\thesubsubsubsubsubsection=0
\ifnum#1=5\global\advance\thesubsubsubsubsection 1
\else
\thesubsubsubsubsection=0
\ifnum#1=4\global\advance\thesubsubsubsection 1
\else
\thesubsubsubsection=0
\ifnum#1=3\global\advance\thesubsubsection 1
\else
\thesubsubsection=0
\ifnum#1=2\global\advance\thesubsection 1
\else
\thesubsection=0
\ifnum#1=1\global\advance\thesection 1
\else
\thesection=0
\global\advance\thechapter 1
\fi\fi\fi\fi\fi\fi}
\def\ppcounter#1{\ifnum#1>0.#1\fi}
\def\ppsection#1{\ifnum#1>0{\ifnum\the\thechapter>0.#1\else#1\fi}\fi}
\def\printcurrentsection{{\bf
\ppcounter{\the\thechapter}\ppsection{\the\thesection}\ppcounter{\the\thesubsection}%
\ppcounter{\the\thesubsubsection}\ppcounter{\the\thesubsubsubsection}%
\ppcounter{\the\thesubsubsubsubsection}\ppcounter{\the\thesubsubsubsubsubsection}}}
%\ZZ{title}{depth}{sec}{page}{ss}
\def\contentsline#1#2#3#4{\ifnum#1=0 \smallbreak\fi
  \line{\incr#1\printcurrentsection~#2
    \rm\leaders\hbox to .5em{.\hfil}\hfil\ #3\hbox to3em{\hss#4}}}


\resetsectioncounters

\def\EQ{\hbox to1em{\vbox{\hbox{$\Delta$}\hbox{$=$}}}}
\def\PE{$+$\EQ}
\def\m#1{{\bf #1}}
\def\a{\rightskip=0pt plus 100pt minus 10pt }